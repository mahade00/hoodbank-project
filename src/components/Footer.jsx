...
const Footer = () =>  (
        <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
            <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full `}>
                ...
                <img src={logo} alt='hoobank' className='w-[266px] h-[72px] objetct-contain' />
                    ...
                    <div key={footerLink.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
                        ...
                        <ul className='list-none mt-4'>
                            {footerLink.links.map((link, index) => (
                                <li key={link.key} className={`font-poppins fonts-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index!== footerLink.links.length -1?  'mb-4': 'mb-0'}`}>
                                    {link.name}
                                </li>
                            ))}
                        </ul>
